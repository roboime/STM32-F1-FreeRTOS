...
SPI_STM32::SPI_STM32(SPI_HandleTypeDef &spi, IO_Pin &SS_PIN):
	_spi(&spi),
	_SS_PIN(&SS_PIN)
{
//    if(_spi==SPI1){
...
//	SPI_CalculateCRC(_spi, DISABLE);
	__HAL_SPI_ENABLE(_spi); // Enable no SPE
	//SPI_Cmd(_spi, ENABLE);
//	Release();
}
...
uint8_t SPI_STM32::WriteBuffer(uint8_t* buffer, uint16_t size) {
	...
	while(size--){
		...
		while(__HAL_SPI_GET_FLAG(_spi,SPI_FLAG_TXE) == RESET){  //SPI_I2S_GetFlagStatus(_spi, SPI_I2S_FLAG_TXE) == RESET
			if(timeout_t--==0) return Timeout();
		}

		_spi->Instance->DR=*buffer++;
		//SPI_I2S_SendData(_spi, *buffer++);

		timeout_t=SPI_TIMEOUT;
		while (__HAL_SPI_GET_FLAG(_spi,SPI_FLAG_BSY) == SET){ //SPI_I2S_GetFlagStatus(_spi, SPI_I2S_FLAG_BSY) == SET
			if(timeout_t--==0) return Timeout();
		}

		timeout_t=SPI_TIMEOUT;
		while (__HAL_SPI_GET_FLAG(_spi,SPI_FLAG_RXNE) == RESET){ //SPI_I2S_GetFlagStatus(_spi, SPI_I2S_FLAG_RXNE) == RESET
			if(timeout_t--==0) return Timeout();
		}
		resp += _spi->Instance->DR;
